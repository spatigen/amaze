...
class MazePackedDataset(PackedDataset):
 ...
 #       return self.overfit_sample
    ...
    def maze_sample_to_sequence_plan(self, maze_sample):
        ...
        """
        m_original_img = maze_sample['m_original_img']  # 带标记的原始图像（用于图像编辑）
        sol_img = maze_sample['sol_img']  # 目标解答图像
        mask_img = maze_sample['mask_img']  # mask图像
        ...
        if mask_img is not None:
            ...
            # Apply VAE transform to mask to match target image size
            if self.vae_transform is not None:
                mask_tensor = self.vae_transform(mask_img)
                # Convert to binary mask (0 or 1)
                # Assuming mask is white (255) where solution exists
                mask_tensor = (mask_tensor.mean(dim=0, keepdim=True) > 0.5).float()  # [1, H, W]

                # Debug: Log mask info for the first few samples
            #    if hasattr(self, '_debug_mask_count'):
             #       self._debug_mask_count += 1
              #  else:
               #     self._debug_mask_count = 1

                #if self._debug_mask_count <= 3:
                 #   print(f"[Dataset Debug] Sample {self._debug_mask_count}: mask_tensor shape={mask_tensor.shape}, "
                  #        f"sum={mask_tensor.sum().item():.1f}, "
                   #       f"has_white={(mask_tensor > 0.5).any().item()}")
      #  else:
       #     print(f"[Dataset Debug] mask_img is None!")
        ...
        # 1. Add marked original image (for understanding) - 使用带标记的图像作为输入
        ...
        # 2. Add conditioning image (for generation) - 使用带标记的图像作为条件输入
        if self.vae_transform is not None:
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if len(buffer) > 0 and sequence_status['curr'] < self.prefer_buffer_before:
                ...
                # If sample is too long, add to buffer and skip
