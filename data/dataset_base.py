...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            # Special handling for MazePackedDataset to pass data_config
            if grouped_dataset_name == 'maze_edit':
                dataset = DATASET_REGISTRY[grouped_dataset_name](
                    ...
                    data_config=self.data_config,  # Pass data_config for maze dataset
                    **dataset_args
                )
            else:
                dataset = DATASET_REGISTRY[grouped_dataset_name](
...
class SimpleCustomBatch:
    def __init__(self, batch):
        ...
        # Handle padded_masks if present
        if "padded_masks" in data.keys():
    ...
    def pin_memory(self):
        ...
        if hasattr(self, 'padded_masks'):
            self.padded_masks = self.padded_masks.pin_memory()
    ...
    def cuda(self, device):
        ...
        if hasattr(self, 'padded_masks'):
            self.padded_masks = self.padded_masks.to(device)
    ...
    def to_dict(self):
        ...
        if hasattr(self, 'padded_masks'):
            data['padded_masks'] = self.padded_masks
