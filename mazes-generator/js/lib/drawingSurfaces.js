    ...
    canvas(config) {
        ...
        return {
            ...
            setSpaceRequirements(requiredWidth, requiredHeight, shapeSpecificLineWidthAdjustment = 1) {
                // 使用固定的线宽和缩放比例，而不是动态计算
                // 这确保了格子大小和墙壁宽度始终固定
                const FIXED_LINE_WIDTH = 10.24;  // 固定墙壁宽度
                const FIXED_CELL_SIZE = 102.4;    // 固定格子大小

                magnification = FIXED_CELL_SIZE;
                ctx.lineWidth = FIXED_LINE_WIDTH;
                ctx.lineCap = 'round';
                xOffset = FIXED_LINE_WIDTH / 2;
                yOffset = FIXED_LINE_WIDTH / 2;
            },
            setColour(colour) {
