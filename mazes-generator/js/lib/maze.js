    ...
    grid.render = function(drawingSurface = defaultDrawingSurface) {
        ...
        if (path) {
            ...
            for (let i = 0; i < path.length; i++) {
                ...
                // 第一个格子（入口）：路径从格子中心（入口标志圆圈位置）开始
                if (i === 0) {
                    currentX = midPoint(p1x, p2x, p3x);
                    currentY = midPoint(p1y, p2y, p3y);
                }
                // 最后一个格子（出口）：路径在格子中心（出口标志叉号位置）结束
